...
<x-filament::dropdown placement="bottom-end">
    ...
    <x-filament::dropdown.header :color="$accountItem?->getColor() ?? 'secondary'" :icon="$accountItem?->getIcon() ?? 'heroicon-s-user-circle'" :href="$accountItemUrl" :tag="filled($accountItemUrl) ? 'a' : 'div'">
    ...
    <x-filament::dropdown.list x-data="{
        ...
        @foreach ($items as $key => $item)
            @if ($key !== 'account' && $key !== 'logout')
                <x-filament::dropdown.list.item :color="$item->getColor() ?? 'secondary'" :icon="$item->getIcon()" :href="$item->getUrl()" tag="a">
                    {{ __($item->getLabel()) }}
                </x-filament::dropdown.list.item>
            @endif
        @endforeach

        <x-filament::dropdown.list.item :color="$logoutItem?->getColor() ?? 'secondary'" :icon="$logoutItem?->getIcon() ?? 'heroicon-s-logout'" :action="$logoutItem?->getUrl() ?? route('filament.auth.logout')" method="post"
            tag="form">
