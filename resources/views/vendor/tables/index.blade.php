...
<div class="filament-tables-component" x-data="{
    ...
    <x-tables::container>
        <div class="filament-tables-header-container" x-show="hasHeader = (@js($renderHeader = $header || $heading || ($headerActions && !$isReordering) || $isReorderable || $isGlobalSearchVisible || $hasFilters || $isColumnToggleFormVisible) || selectedRecords.length)"
            ...
            @elseif ($heading || $headerActions)
                <div @class(['px-2 pt-2', 'hidden' => !$heading && $isReordering])>
                    <x-tables::header class="mb-2" :actions="$isReordering ? [] : $headerActions">
                        <x-slot name="heading">
                            {{ $heading }}
                        </x-slot>
            ...
            <div class="filament-tables-header-toolbar flex items-center justify-between p-2 h-14"
                x-show="@js($shouldRenderHeaderDiv = $isReorderable || $isGlobalSearchVisible || $hasFiltersPopover || $isColumnToggleFormVisible) || selectedRecords.length" {!! !$shouldRenderHeaderDiv ? 'x-cloak' : null !!}
                x-bind:class="{
                ...
                @if ($isGlobalSearchVisible || $hasFiltersPopover || $isColumnToggleFormVisible)
                    <div class="flex items-center justify-end w-full gap-2 md:max-w-md">
                        @if ($isGlobalSearchVisible)
                            <div class="filament-tables-search-container flex items-center justify-end flex-1 gap-3">

                                {{-- ! Changed by RawanD201 --}}
                                @php
                                    $cachedBA = $this->getCachedTableBulkActions();
                                    $exportKey = method_exists($this, 'getExportBulkActionKey') ? $this->getExportBulkActionKey() : 'export';
                                @endphp
                                @if (\Illuminate\Support\Arr::exists($cachedBA, $exportKey))
                                    <div class="shrink-0">
                                        <x-filament::pages.actions x-show="selectedRecords.length" :actions="[$this->getCachedTableBulkAction($exportKey)]" />
                                    </div>
                                @endif
                        ...
                        @if ($hasFiltersPopover)
                            <x-tables::filters.popover class="shrink-0" :form="$getFiltersForm()" :max-height="$getFiltersFormMaxHeight()"
                                :width="$getFiltersFormWidth()" :indicators-count="count(\Illuminate\Support\Arr::flatten($filterIndicators))" />
                        @endif

                        @if ($isColumnToggleFormVisible)
                            <x-tables::toggleable class="shrink-0" :form="$getColumnToggleForm()" :max-height="$getColumnToggleFormMaxHeight()"
        ...
        @elseif ($isSelectionEnabled && $isLoaded)
            <x-tables::selection-indicator :all-selectable-records-count="$allSelectableRecordsCount" :colspan="$columnsCount" x-show="selectedRecords.length"
                ...
                @if (($content || $hasColumnsLayout) && !$isReordering)
                    ...
                    <div @class([
                        ...
                        @if (count($sortableColumns))
                            <div class="flex flex-wrap items-center gap-1 py-1 text-xs sm:text-sm"
                                x-data="{
                                ...
                                <label>
                                    ...
                                    <select style="background-position: right 0.2rem center" x-model="column"
                                        @class([
                                ...
                                <label>
                                    ...
                                    <select style="background-position: right 0.2rem center" x-show="column"
                                        x-model="direction" @class([
                                            'text-xs pl-2 pr-6 py-1 font-medium border-0 bg-gray-500/5 rounded-lg border-gray-300 sm:text-sm focus:ring-0 focus:border-primary-500 focus:ring-primary-500',
                                            'dark:text-white dark:bg-gray-700 dark:border-gray-600 dark:focus:border-primary-500' => config(
                                                'tables.dark_mode'),
                                        ])>
                    ...
                    <x-filament-support::grid wire:sortable
                        ...
                        @foreach ($records as $record)
                            ...
                            <div @if ($hasCollapsibleColumnsLayout) x-data="{ isCollapsed: @js($collapsibleColumnsLayout->isCollapsed()) }"
                                    ...
                                    x-on:collapse-all-table-rows.window="isCollapsed = true" @endif
                                ...
                                <div x-bind:class="{
                                    ...
                                    <div @class([
                                        ...
                                        @if ($hasCollapsibleColumnsLayout)
                                            <div @class([
                                                ...
                                                <x-tables::icon-button class="transition" icon="heroicon-s-chevron-down"
                                        ...
                                        @if ($recordUrl)
                                            <a class="filament-tables-record-url-link flex-1 block py-3"
                                                href="{{ $recordUrl }}">
                                        ...
                                        @elseif ($recordAction)
                                            ...
                                            <button class="filament-tables-record-action-button flex-1 block py-3"
                                                type="button" wire:click="{{ $recordWireClickAction }}"
                                    ...
                                    @if ($hasCollapsibleColumnsLayout)
                                        <div x-show="! isCollapsed" x-collapse x-cloak @class([
                                            'pb-2 -mx-2',
                    ...
                    <x-slot name="header">
                        ...
                        @foreach ($columns as $column)
                            <x-tables::header-cell
                                ...
                                :extra-attributes="$column->getExtraHeaderAttributes()" :is-sort-column="$getSortColumn() === $column->getName()" :name="$column->getName()" :alignment="$column->getAlignment()"
                    ...
                    @if ($records !== null && count($records))
                        @foreach ($records as $record)
                            ...
                            <x-tables::row :record-action="$recordAction" :record-url="$recordUrl"
                                ...
                                @if ($isSelectionEnabled && $recordCheckboxPosition === RecordCheckboxPosition::BeforeCells)
                                    @if ($isRecordSelectable($record))
                                        <x-tables::checkbox.cell :class="\Illuminate\Support\Arr::toCssClasses([
                                            ...
                                            <x-tables::checkbox class="filament-tables-record-checkbox"
                                                x-model="selectedRecords" :value="$recordKey" :label="__('tables::table.fields.bulk_select_record.label', [
                                                    'key' => $recordKey,
                                                ])" />
                                        </x-tables::checkbox.cell>
                                    @else
                                ...
                                <x-tables::loading-cell class="hidden" :colspan="$columnsCount"
            ...
            @else
                ...
                @else
                    <div class="flex items-center justify-center p-4">
                        <x-tables::empty-state :icon="$getEmptyStateIcon()" :actions="$getEmptyStateActions()" :column-searches="$hasColumnSearches()">
                            <x-slot name="heading">
    ...
    <form wire:submit.prevent="callMountedTableAction">
        ...
        <x-tables::modal :id="$this->id . '-table-action'"
            ...
            :visible="filled($action)" :width="$action?->getModalWidth()" :slide-over="$action?->isModalSlideOver()" :close-by-clicking-away="$action?->isModalClosedByClickingAway()" display-classes="block"
